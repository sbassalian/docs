...
| `data[].approved_at`           | String  | Timestamp when the ambassador application was approved (ISO8601 format)       |
...
### Approved Date Range Filter

Filter ambassadors by their approval date. Both parameters are optional and can be used independently or together:

```bash
# Ambassadors approved in 2024
curl -X GET "https://api.growi.io/api/public/v1/stats/query_ambassadors?start_date=09/01/2025&end_date=09/30/2025&campaign_id=123&approved_at_from=2024-01-01&approved_at_to=2024-12-31" \
     -H "Authorization: Bearer YOUR_PUBLIC_API_KEY"

# Ambassadors approved after a specific date
curl -X GET "https://api.growi.io/api/public/v1/stats/query_ambassadors?start_date=09/01/2025&end_date=09/30/2025&campaign_id=123&approved_at_from=2024-06-01" \
     -H "Authorization: Bearer YOUR_PUBLIC_API_KEY"
```
