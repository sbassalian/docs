...
| `export`                 | String  | Set to "true" to export results as CSV                              | No       |

## Request Example

```bash
curl -X GET "https://api.growi.io/api/public/v1/transactions?page=1&per_page=20&transaction_type=payout&start_date=10/01/25&end_date=10/31/25" \
  ...
  "data": [
    {
      ...
      "invoice": {
        ...
        "line_items": [
          {
            "id": 40958,
            "amount": 1250,
            "user_name": "Sophia Ericksen",
            "campaign_name": "SV x M3",
            "user_id": 162177,
            "order_id": null,
        ...
        "organization_slug": "m3-naturals-111f5516",
        "organization_name": "M3 Naturals",
      ...
      "organization_slug": "m3-naturals-111f5516",
      "invoice_description": "brand manager - mo #2 - 50% upfront - retainer bump"
    }
  ],
  "meta": {
    "row_count": 42,
...
When `export=true` is specified, the endpoint returns CSV data instead of JSON:

```bash
curl -X GET "https://api.growi.io/api/public/v1/transactions?export=true" \
     -H "Authorization: Bearer YOUR_PUBLIC_API_KEY" \
     -H "Content-Type: application/json"
```
...
```json
{
  "csv_data": "ID,Type,Amount,Status,Created,Description\n5525,payout,12.50,paid,1759952556,brand manager - mo #2\n...",
  "filename": "transactions-20251009143022.csv",
  "type": "text/csv"
}
```
