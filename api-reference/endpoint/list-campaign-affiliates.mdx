...
This is a read-only `GET`, so **viewer (read-only) API keys** can call it. Keys are scoped to a single organization, and results always come from the key's organization. Rate limits are on the [Authentication](/authentication) page.
